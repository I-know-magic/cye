...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            orderTable = new EasyUIExt($("#mainGrid"), "<g:createLink controller="tableInfo" action="list" base=".." />?areaId=" + areaId);
        ...
        function myAdd() {
            var areaId=$("#hidden_areaId").val();
            var areaName=$("#hidden_areaName").val();
            orderTable.mainAdd("<g:createLink controller="tableInfo" action="create"  base=".." />?areaId="+areaId+"&areaName="+areaName, "桌台增加");
            orderTable.formAction = "<g:createLink controller="tableInfo" action="save"  base=".."  />";
        }
        function edit(id) {
            orderTable.mainEdit("<g:createLink controller="tableInfo" action="edit"  base=".."  />", "桌台修改", id);
            orderTable.formAction = "<g:createLink controller="tableInfo" action="update"  base=".."  />";
        }
        function del() {
            orderTable.mainDel("<g:createLink controller="tableInfo" action="delete" base=".."   />");
        }
        function doSearch() {
            $("#mainGrid").datagrid({
                queryParams: {
...
<input type="hidden" id="hidden_areaId" value="${areaId}">
<input type="hidden" id="hidden_areaName" value="${areaName}">
...
<div class="rel clearfix function-btn">
    <ul class="boxtab-btn abs">
        ...
        <li class="icon back class_branch_op" onclick="callBack()">返回</li>
    </ul>
...
<div class="table-list">
    <div class="table-list-r-1 fr" style="background:#b6b6b6;">
        <table id="mainGrid"
            ...
            <thead>
                <th data-options="field:'areaId'" hidden="hidden">区域id</th>
