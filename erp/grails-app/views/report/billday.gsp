...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            orderTable = new EasyUIExt($("#mainGrid"), "<g:createLink controller="billday" action="list" base=".." />");
        ...
        function myAdd() {
            orderTable.mainAdd("<g:createLink  base=".." controller="billday" action="create"/>", "-增加");
            orderTable.formAction = "<g:createLink  base=".." controller="billday" action="save"  />";
        }
        function edit(id) {
            orderTable.mainEdit("<g:createLink  base=".." controller="billday" action="edit"  />", "-修改", id);
            orderTable.formAction = "<g:createLink  base=".." controller="billday" action="update"  />";
        }
        function del() {
            orderTable.mainDel("<g:createLink  base=".." controller="billday" action="delete"  />");
        }
        function doSearch() {
...
<div class="rel clearfix function-btn">
    <ul class="boxtab-btn abs">
        <li class="icon daochu "
            onclick="exportExcel(event, '<g:createLink base=".." controller="billday" action="exportExcel"  />')">导出</li>
    ...
    <p  class="search search-width abs">
        <input type="text" id="queryStr" name="queryStr" placeholder="输入单号或桌台查询" class="search-txt search-txt-width abs  js_isFocus">
    ...
    <div class="table-list-r-1 fr" style="background:#b6b6b6;">
        <table id="mainGrid"
            ...
            <thead>
                ...
                <th data-options="field:'billNo',width:120">单号</th>
                <th data-options="field:'tableName',width:80">桌台</th>
                <th data-options="field:'catName',width:100">类别</th>
                <th data-options="field:'goodsName',width:120">品名</th>
                <th data-options="field:'taste',width:200">口味</th>
                <th data-options="field:'price',width:80">价格</th>
                <th data-options="field:'qty',width:80">数量</th>
                <th data-options="field:'sum',width:120">金额</th>
                <th data-options="field:'payTime',width:120">付款时间</th>
