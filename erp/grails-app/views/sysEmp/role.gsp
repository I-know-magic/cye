    ...
    $(function () {
        ...
        orderTable.loadSuccess = function (data) {
            $.ajax({
                type: 'GET',
                async: true,
                url: '<g:createLink base=".." controller='sysEmp' action='selectedRole'/>?userId=' + userId,
                dataType: 'json',
        ...
        $.messager.confirm('系统提示', '您确认给此用户设置所选的这些角色吗？', function (r) {
            ...
            if (r) {
                ...
                $.ajax({
                    type: 'GET',
                    async: true,
                    url: '<g:createLink base=".." controller='sysEmp' action='employeeRoles'/>?roleIds=' + roleIds + '&userId=' + userId,
                    dataType: 'json',
