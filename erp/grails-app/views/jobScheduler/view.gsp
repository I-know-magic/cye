...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            orderTable = new EasyUIExt($("#mainGrid"), "<g:createLink controller="jobScheduler" action="list"  />");
        ...
        function myAdd() {
            orderTable.mainAdd("<g:createLink controller="jobScheduler" action="create"/>","定时任务表-增加");
            orderTable.formAction = "<g:createLink controller="jobScheduler" action="save"  />";
        }
        function edit(id) {
            orderTable.mainEdit("<g:createLink controller="jobScheduler" action="edit"  />", "定时任务表-修改", id);
            orderTable.formAction = "<g:createLink controller="jobScheduler" action="update"  />";
        }
        function del() {
            orderTable.mainDel("<g:createLink controller="jobScheduler" action="delete"  />");
        }
        function doSearch() {
        $("#mainGrid").datagrid({
                queryParams: {
...
<div class="table-list">
    ...
    <th data-options="field:'rate'">频率</th>
    <th data-options="field:'beginTime'"></th>
    <th data-options="field:'type'">类型1-日冻结</th>
        ...
        <div id="editWindow" class="easyui-dialog "
            ...
            <tr>
                ...
                <td><input class="easyui-textbox" type="text" name="rate" data-options="required:true"/></td>
            </tr>
            <tr>
                <td class="title">:</td>
                <td><input class="easyui-textbox" type="text" name="beginTime" data-options="required:true"/></td>
            </tr>
            <tr>
                <td class="title">类型1-日冻结:</td>
