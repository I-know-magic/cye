...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            orderTable = new EasyUIExt($("#mainGrid"), "<g:createLink controller="category" action="list"  />");
        ...
        function myAdd() {
            orderTable.mainAdd("<g:createLink controller="category" action="create"/>","商品分类-增加");
            orderTable.formAction = "<g:createLink controller="category" action="save"  />";
        }
        function edit(id) {
            orderTable.mainEdit("<g:createLink controller="category" action="edit"  />", "商品分类-修改", id);
            orderTable.formAction = "<g:createLink controller="category" action="update"  />";
        }
        function del() {
            orderTable.mainDel("<g:createLink controller="category" action="delete"  />");
        }
        function doSearch() {
        $("#mainGrid").datagrid({
                queryParams: {
...
<div class="table-list">
    <div class="table-list-r-1 fr" style="background:#b6b6b6;">
        <table id="mainGrid"
            ...
            <thead>
                <th data-options="field:'catCode'">商品分类码</th>
    <th data-options="field:'catName'">商品分类名称</th>
    <th data-options="field:'mnemonics'">助记码</th>
    <th data-options="field:'parentId'">上级分类ID
外键</th>
    <th data-options="field:'categoryType'"></th>
            ...
            <form id="editForm" method="post">
                <table cellpadding="5" style="table-layout:fixed;">
                    <input class="easyui-validatebox" type="hidden" name="id" id="id"/>
                                <tr>
                <td class="title">商品分类码:</td>
                <td><input class="easyui-textbox" type="text" name="catCode" data-options="required:true"/></td>
            </tr>
            <tr>
                <td class="title">商品分类名称:</td>
                <td><input class="easyui-textbox" type="text" name="catName" data-options="required:true"/></td>
            </tr>
            <tr>
                <td class="title">助记码:</td>
                <td><input class="easyui-textbox" type="text" name="mnemonics" data-options="required:true"/></td>
            </tr>
            <tr>
                <td class="title">上级分类ID
外键:</td>
                <td><input class="easyui-textbox" type="text" name="parentId" data-options="required:true"/></td>
            </tr>
            <tr>
                <td class="title">:</td>
                <td><input class="easyui-textbox" type="text" name="categoryType" data-options="required:true"/></td>
            </tr>
